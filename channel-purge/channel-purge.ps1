...
# ── Auto-find channelPurge.tsx ───────────────────────────────────────────────
$pluginSrc = Join-Path $PSScriptRoot "channelPurge.tsx"

if (-not (Test-Path $pluginSrc)) {
    Write-Host "channelPurge.tsx not next to script, searching..." -ForegroundColor Yellow
    $found = Get-ChildItem -Path $env:USERPROFILE -Filter "channelPurge.tsx" -Recurse -ErrorAction SilentlyContinue | Select-Object -First 1
    if ($found) {
        $pluginSrc = $found.FullName
        Write-Host "      Found: $pluginSrc" -ForegroundColor Yellow
    } else {
        Write-Error "Could not find channelPurge.tsx anywhere under $env:USERPROFILE"
        exit 1
    }
}
